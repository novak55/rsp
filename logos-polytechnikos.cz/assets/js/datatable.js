...
$( document ).ready(function() {
    var dataTableCZ = {
        ...
        "sInfoEmpty":      "Zobrazuji 0 záznamů",
    ...
    $('.datatable_filter:not(.DataTable)').each((i, el) => {
        let dt = $(el).DataTable({
            paging: true,
        ...
        $(document).trigger("datatable_created", {element: el, api: dt});
    });
...
$(document).on("datatable_created", (e, dtargs) => {
    $("[data-search-in]").each((i, el) => {
        $(el).on("input", () => {
            dtargs.api.search(el.value, false, true);
            dtargs.api.draw();
        });
        $(el).on("input", () => {
            dtargs.api.search(el.value, false, true);
    ...
    $("[data-pagelen-for]").each((i, el) => {
        $(el).on("change", () => {
            console.log(el, dtargs);
            dtargs.api.page.len(el.value).draw();
        });
    });
});
